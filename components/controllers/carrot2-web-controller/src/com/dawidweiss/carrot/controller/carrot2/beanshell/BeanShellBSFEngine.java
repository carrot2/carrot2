...
package com.dawidweiss.carrot.controller.carrot2.beanshell;
...
/**
 * This is the BeanShell adapter for IBM's Bean Scripting Famework.     It is an implementation of
 * the BSFEngine class, allowing BSF aware     applications to use BeanShell as a scripting
 * language.
 * 
 * <p></p>
 *
 * @author Pat Niemeyer
 * @author Dawid Weiss (modified to org.apache.bsf. from the original IBM's classes).
 */
    ...
    extends BSFEngineImpl
{
    ...
    /**
     * Invoke method name on the specified bsh scripted object. The object may be null to indicate
     * the global namespace of the interpreter.
     *
        ...
        if (object instanceof bsh.This)
        {
            ...
            catch (TargetError e2)
            {
                throw new BSFException(
                    "The application script threw an exception: " + e2.getTarget()
                );
            }
            catch (EvalError e3)
            {
    ...
    /**
     * A helper BeanShell method that implements the anonymous method apply proposed by BSF.  Note
     * that the script below could use the standard bsh eval() method to set the variables and
     * apply the text, however then I'd have to escape quotes, etc.
     */
    ...
    /**
     * This is an implementation of the BSF apply() method. It exectutes the funcBody text in an
     * "anonymous" method call with arguments.
     */
    public Object apply(
        String source, int lineNo, int columnNo, Object funcBody, Vector namesVec, Vector argsVec
    )
        throws BSFException
    {
        ...
        {
            throw new BSFException(
                "BeanShell interpreter internal error: " + e + sourceInfo(source, lineNo, columnNo)
            );
        }
        catch (TargetError e2)
        {
        ...
        {
            throw new BSFException(
                "BeanShell script error: " + e3 + sourceInfo(source, lineNo, columnNo)
            );
        }
    }
        ...
        throws BSFException
    {
        ...
        catch (TargetError e2)
        {
            throw new BSFException(
                ...
                "BeanShell script threw an exception: " + sourceInfo(source, lineNo, columnNo),
                e2.getTarget()
            );
        }
        catch (EvalError e3)
