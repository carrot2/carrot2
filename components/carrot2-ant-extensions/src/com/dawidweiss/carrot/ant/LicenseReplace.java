...
/**
 * Replaces licensing information according to the given
 * rules.
 * 
 * @author Dawid Weiss
 */
public class LicenseReplace extends AbstractLicenseTask {

    /**
     * A replaceable block contains a key which detects the
     * header to be replaced ({@link #setContains(String)}) and
     * a replacement ({@link #setReplacement(String)}) which replaces it.  
     */
    public abstract class ReplaceableBlock {
        ...
        protected void checkAttributes() throws BuildException {
            if (empty && containsKey != null) {
                throw new BuildException("Attributes empty and contains are mutually exclusive.");
            }
    ...
    public class HeaderBlock extends ReplaceableBlock {
        public SourceFile process(SourceFile original) {
            ...
            if (checkReplace(original.getCanonicalizedHeader())) {
                return new SourceFile(super.replacement, original.getContent(), original.getFooter());
            } else {
                return null;
            }
        }
    ...
    public class FooterBlock extends ReplaceableBlock {
        public SourceFile process(SourceFile original) {
            ...
            if (checkReplace(original.getCanonicalizedFooter())) {
                return new SourceFile(original.getHeader(), original.getContent(), super.replacement);
            } else {
                return null;
            }
        }
    ...
    /**
     * Number of processed files using each of the
     * {@link ReplaceableBlock}s in {@link #blocks} variable.
     */
    ...
    protected void checkLicense(File file) throws IOException {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if (newSplit != null) {
                ...
                writeFile(newSplit.recreateFile(), new File(file.getAbsolutePath() + saveExtension));
            }
        }
    }
}
