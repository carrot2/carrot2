...
public class HTMLNode  {
   ...
   * @param num the ordinary number of the node
   */
  public HTMLNode(HTMLTree owner, TagToken tag, HTMLNode parent, Enumeration src, int level, int num[]) {
  ...
  private void processPossibleStandAlone(Vector possibleStandAlone, Vector store, TagToken tag) {
    for (int i = possibleStandAlone.size() - 1; i >= 0; i--) {
      if ( ( (HTMLNode) possibleStandAlone.get(i)).getName().equals(tag.getName())) {
        ((HTMLNode) possibleStandAlone.get(i)).assignChildren(store,
            store.indexOf(possibleStandAlone.get(i)) + 1, store.size() - 1);
...
/*            Enumeration e = possibleStandAlone.elements();
  ...
  private static String[] defaultStandaloneList = {
      ...
      "link", "meta", "nextid", "option", "overlay",
      "param", "tab", "wbr", "!", "!--", "!DOCTYPE"
  };
  ...
  /**
   * String of default node names which can be standalone and should be closed before the next occurence.
   */
