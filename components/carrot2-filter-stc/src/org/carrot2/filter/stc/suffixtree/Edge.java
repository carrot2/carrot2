 ...
 * 
 * A single edge may represent more than one element of SuffixableElement
 * collection (otherwise it would be a trie not a suffix tree).
 * 
 * @author Dawid Weiss
 */
public class Edge {
    ...
    /**
     * This method splits an Edge on a given suffix, creates a new Node and
     * makes a fork in the tree.
     */
    protected Node splitEdge(Suffix s) {
        startNode.removeEdge(this);

        Edge newEdge = s.originNode.createChildNode(s.firstElementIndex,
                s.lastElementIndex);

        newEdge.endNode.suffixNode = s.originNode;

        if (newEdge.endNode.suffixNode == null)
