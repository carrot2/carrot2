 ...
 *
 */
public class DummyLcpSuffixSorter
    extends AbstractLcpSuffixSorter
    implements LcpSuffixSorter
{
     ...
     */
    private void suffixQuickSort(int [] suffixArray, int [] suffixData, int left, int right)
    {
        if ((right - left) < 2)
        {
            return;
        ...
        while ((r - l) > 0)
        {
            while ((r >= left) && (compareSuffixes(suffixArray[r], centerSuffix, suffixData) > 0))
            {
                r--;
            }

            while ((l <= right) && (compareSuffixes(suffixArray[l], centerSuffix, suffixData) < 0))
            {
                l++;
            }
    ...
    private void dummyLcp(int [] suffixArray, int [] lcpArray, int [] suffixData)
    {
        ...
        for (int i = 1; i < suffixArray.length; i++)
        {
            ...
            while (suffixData[suffixArray[i] + j] == suffixData[suffixArray[i - 1] + j])
            {
                lcpArray[i]++;
                j++;
            }
