          ...
          <PointedTabs selectedTabId={activeView} onChange={onViewChange}>
            {
              views.map(group => {
                const components = [];
                if (group.label) {
                  components.push(<div className="TabGroupLabel"
                                       key={group.label}>{group.label}</div>);
                }

                return (components.concat(
                        Object.keys(group.views).map(v => {
                          return (
                              <Tab key={v} id={v} title={allViews[v].label} />
                          );
                        }))
                );
              })
            ...
            viewKeys
                ...
                .reduce(function (tools, v) {
                  allViews[v].tools.forEach(t => {
                    const key = v + "." + t.id;
                    tools.push(<Tool key={key} tool={t} visible={activeView === v}
                                     props={props} />);
                  });
                  return tools;
                }, [])
