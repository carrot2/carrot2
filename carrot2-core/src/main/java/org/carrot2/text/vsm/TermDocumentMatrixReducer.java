...
public class TermDocumentMatrixReducer extends AttrComposite {
  ...
  {
    attributes.register("factorizationFactory",
  ...
  public void reduce(ReducedVectorSpaceModelContext context, int dimensions) {
    ...
    if (vsmContext.termDocumentMatrix.columns() == 0 || vsmContext.termDocumentMatrix.rows() == 0) {
      context.baseMatrix = new DenseDoubleMatrix2D(vsmContext.termDocumentMatrix.rows(),
          vsmContext.termDocumentMatrix.columns());
      return;
    }

    if (factorizationFactory instanceof IterativeMatrixFactorizationFactory) {
      ((IterativeMatrixFactorizationFactory) factorizationFactory).estimateIterationsNumber(dimensions,
          vsmContext.termDocumentMatrix);
    }

    MatrixUtils.normalizeColumnL2(vsmContext.termDocumentMatrix, null);
    final MatrixFactorization factorization = factorizationFactory.factorize(vsmContext.termDocumentMatrix);
  ...
  private final DoubleMatrix2D trim(MatrixFactorizationFactory factorizationFactory, DoubleMatrix2D matrix,
                                    int dimensions) {
