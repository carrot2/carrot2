...
public class AttributeBinder
{
    ...
    private final static Predicate<Field> CONSISTENCY_CHECKS =
        new ConsistencyCheckRequiredAnnotations().and(
        ...
        throws InstantiationException, AttributeBindingException
    {
        return set(object, values, checkRequired,
            filteringAnnotations.length > 0 ?
                new AllAnnotationsPresentPredicate(filteringAnnotations) : (v) -> true);
    }
    ...
    public static void bind(Object object,
        ...
        throws InstantiationException, AttributeBindingException
    {
        bind(new HashSet<>(), new BindingTracker(), 0, object,
            attributeBinderActions,
            CONSISTENCY_CHECKS.and(predicate), Bindable.class);
    }
    ...
    public static class AttributeTransformerFromString implements IAttributeTransformer
    {
        ...
        public Object transform(Object value, String key, Field field)
        {
            ...
            final Class<?> fieldType = AttributeUtils.toBoxed(field.getType());
        ...
        private Object callValueOf(final String stringValue, final Class<?> fieldType)
        {
            ...
            catch (InvocationTargetException e)
            {
                ...
                {
                    throw new RuntimeException(target); // previously propagate
                }
            }
        }
    }
    ...
    public static class AttributeBinderActionBind implements IAttributeBinderAction
    {
            ...
            boolean checkRequired, IAttributeTransformer... transformers)
        {
            ...
            this.remainingValues = new HashMap<>(values);
        }
    ...
    {
        static Set<Class<?>> ALLOWED_PLAIN_TYPES = Collections.singleton(File.class);

        static Set<Class<?>> ALLOWED_ASSIGNABLE_TYPES = new HashSet<>(Arrays.asList(
        ...
        public boolean test(Field field)
        {
            ...
            final Class<?> attributeType = AttributeUtils.toBoxed(field.getType());
    ...
    public static final class BindingTracker
    {
         ...
         */
        private Map<String, Integer> bindingLevel = new HashMap<>();

        /**
         * Containing instance + attribute key pairs that have already been collected.
         */
        private Set<Pair<Object, String>> boundInstances = new HashSet<>();
