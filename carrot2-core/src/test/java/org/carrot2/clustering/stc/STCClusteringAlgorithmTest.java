...
public class STCClusteringAlgorithmTest extends ClusteringAlgorithmTestBase {
  ...
  public void testComputeIntersection() {
    ...
    Assertions.assertThat(STCClusteringAlgorithm.computeIntersection(t1, 0, 3, t1, 3, 3)).isEqualTo(2);

    t1 = new int[]{0, 1, 2, 3, 5, 6};
    Assertions.assertThat(STCClusteringAlgorithm.computeIntersection(t1, 0, 3, t1, 3, 3)).isEqualTo(0);

    t1 = new int[]{0, 1, 2, -1, 2, 6};
    Assertions.assertThat(STCClusteringAlgorithm.computeIntersection(t1, 0, 3, t1, 3, 3)).isEqualTo(1);

    t1 = new int[]{0, 1, 2, 0};
    Assertions.assertThat(STCClusteringAlgorithm.computeIntersection(t1, 0, 3, t1, 3, 1)).isEqualTo(1);
  }
  ...
  public void testCarrot1008() throws Exception {
    ...
    Stream<TestDocument> docStream = elementStream(root, "document")
        .map(docElement -> {
          String title = elementStream(docElement, "title")
              .map(e -> e.getTextContent()).collect(Collectors.joining(" . "));
          String snippet = elementStream(docElement, "snippet")
              .map(e -> e.getTextContent()).collect(Collectors.joining(" . "));
    ...
    Assertions.assertThat(collect.contains("Guns") &&
                          collect.contains("Gun")).isFalse();
  }
