...
/**
 * The ImageCombo class represents a selectable user interface object that combines a text
 * field and a table and issues notification when an item is selected from the table.
 * <p>
 * <b>GTK Corrections:</b>
 * <dl>
 ...
 * standard "ClearLooks" which shows the gray background between Text and Arrow and border</dd>
 ...
 * </p>
 * <p>
 * Note that although this class is a subclass of <code>Composite</code>, it does not
 * make sense to add children to it, or set a layout on it.
 * </p>
 * <dl>
 * <dt><b>Styles:</b>
...
public final class ImageCombo extends Composite
{
     ...
     * </p>
     * 
     * @param parent a widget which will be the parent of the new instance (cannot be
     *            null)
    ...
    {
        int mask =
            gtk ? SWT.READ_ONLY | SWT.FLAT | SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT
     ...
     *                number of elements in the list (inclusive)</li>
     *                </ul>
     * @exception SWTException
     *                <ul>
     *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed</li>
     ...
     * receiver's text is modified, by sending it one of the messages defined in the
     ...
     * <code>widgetDefaultSelected</code> is typically called when ENTER is pressed the
     * combo's text area.
     * </p>
     * 
     * @param listener the listener which should be notified
    ...
    /**
     * Sets the selection in the receiver's text field to an empty selection starting just
     * before the first character. If the text field is editable, this has the effect of
     * placing the i-beam at the start of the text.
     * <p>
    ...
    public Point computeSize(int wHint, int hHint, boolean changed)
    {
        ...
        width =
            Math.max(wHint, Math.max(textWidth + 2 * spacer + arrowSize.x + 2
    ...
    void createPopup(int selectionIndex)
    {
        ...
        int [] listEvents =
            {
                SWT.MouseUp, SWT.Selection, SWT.Traverse, SWT.KeyDown, SWT.KeyUp,
    ...
    /**
     * Deselects the item at the given zero-relative index in the receiver's list. If the
     * item at the index was already deselected, it remains deselected. Indices that are
    ...
    void dropDown(boolean drop)
    {
        ...
        itemCount =
            (itemCount == 0) ? visibleItemCount : Math.min(visibleItemCount, itemCount);
    ...
    /*
     * Return the Label immediately preceding the receiver in the z-order, or null if
     * none.
     */
    ...
    /**
     * Returns the item at the given, zero-relative index in the receiver's list. Throws
    ...
    /**
     * Returns the height of the area which would be used to display <em>one</em> of the
    ...
    /**
     * Returns an array of <code>String</code>s which are the items in the receiver's
     * list.
     * <p>
     * Note: This is not the actual structure used by the receiver to maintain its list of
    ...
    /**
     * Returns a <code>Point</code> whose x coordinate is the start of the selection in
     * the receiver's text field, and whose y coordinate is the end of the selection. The
     * returned values are zero-relative. An "empty" selection as indicated by the the x
    ...
    /**
     * Returns the maximum number of characters that the receiver's text field is capable
     * of holding. If this has not been changed by <code>setTextLimit()</code>, it will
     * be the constant <code>Combo.LIMIT</code>.
     * 
    ...
    /**
     * Searches the receiver's list starting at the first item (index 0) until an item is
     * found that is equal to the argument, and returns the index of that item. If no item
     * is found, returns -1.
     * 
     ...
     *                between 0 and the number of elements in the list minus 1 (inclusive)</li>
     *                </ul>
     * @exception SWTException
     *                <ul>
     *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed</li>
    ...
    /**
     * Searches the receiver's list starting at the first item until an item is found that
     * is equal to the argument, and removes that item from the list.
     * 
    ...
    /**
     * Selects the item at the given zero-relative index in the receiver's list. If the
     * item at the index was already selected, it remains selected. Indices that are out
    ...
    /**
     * Sets the text of the item in the receiver's list at the given zero-relative index
     * to the string argument. This is equivalent to <code>remove</code>'ing the old
     * item at the index, and then <code>add</code>'ing the new item at that index.
     * 
     * @param index the index for the item
    ...
    /**
     * Sets the layout which is associated with the receiver to be the argument which may
     * be null.
     * <p>
     * Note : No Layout can be set on this Control because it already manages the size and
     ...
     * argument whose x coordinate is the start of the selection and whose y coordinate is
     ...
     * displaying a single line of text. Thus, setting the text to a string containing
     * line breaks or other special characters will probably cause it to display
     * incorrectly.
     * </p>
     * 
    ...
    /**
     * Sets the maximum number of characters that the receiver's text field is capable of
