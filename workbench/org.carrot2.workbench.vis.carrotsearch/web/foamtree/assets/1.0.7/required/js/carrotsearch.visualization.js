...
function CarrotSearchVisualization() {
  ...
  // A reference to this delegate
  var thisVisualization = this;

  // A reference to the actual implementation using this delegate. It's not pretty,
  // but saves a lot of hassle when setting "this" in callbacks to the visualization.
  var implementationVisualization;

  // DOM element containing this visualization
  var element = null;

  // Public methods of this delegate used by the actual implementations
  this.init = init;
   ...
   */
  function init(implementation, visualizationConfig) {
    implementationVisualization = implementation;
    config = visualizationConfig;
    
    extend(getOptionMethods, config.getOptionMethods);

    // Temporarily set up options without defaults for initial validation
    options = extend({}, config.settings);
    
    // Validate and substitute deprecated options
    if (canLog()) {
    ...
    // Merge corrected user options with defaults
    options = extend({}, config.defaults, options);

    // Export public methods. The delegate instance is returned
    exportPublicMethodsTo(implementationVisualization);
  }

  /**
   * Exports common public methods to the provided instance.
   */
  function exportPublicMethodsTo(instance) {
    instance.get = get;
    instance.set = set;
    instance.init = init;
    instance.embed = embed;
    instance.print = print;
    instance.expandAll = expandAll;
    instance.expand = expand;
    instance.collapseAll = collapseAll;
    instance.collapse = collapse;
    instance.saveAsImage = saveAsImage;
    instance.updateColors = updateColors;
  }
  ...
  function embed(initial) {
    ...
    each(flashvars, function(name, value) {
      flashvars[name] = encodeURIComponent(value);
    });
  ...
  function prepareFlashVars(skipNulls) {
    var flashvars = { };
    each(options, function(name, optionValue) {
      var key = config.jsToSwfPropertyName[name];
      if (key) {
        var value = config.transformers[name] ? config.transformers[name].call(this, optionValue, name) : optionValue;
    ...
    // Related to: http://issues.carrot2.org/browse/FOAMTREE-111.
    // When the visualization gets re-initialized after the containing HTML
    // element has been hidden/shown, it loses all the internal state. To
    // keep the previous data and options, we need to pass the options remembered
    // in the JavaScript API back to the Flash code. This is done through the
    // private onPreinit event.
    flashvars.onPreinit = asGlobalCallback(function () {
      return prepareFlashVars(false);
    }, "preinit");
  ...
  function set() {
    ...
    if (needsReload) {
      if (element && has(element, "reload")) {
        ...
        // All properties, including dataUrl or dataXml will
        // be set during the reload.
        element.reload(vars);
      } else {
        embed(false);
      }
    } else {
      // Use Flash methods to set data to avoid a reload
      ...
      // Use Flash methods to set reloadless options
      each(setOptionMethods, function(option, method) {
        if (has(opts, option)) {
          method.call(thisVisualization, opts);
        }
      });
    }
  ...
  function select(opts) {
    ...
    if (groups !== null) {
      each(groups, function(group, groupValue) {
        element.selectGroupById(groupValue,
  ...
  function validateOptions(opts, extraOpts, openGroupCallback) {
    if (config.aliases) {
      ...
      each(allAliases, function (removedInVersion, aliases) {
        each(aliases, function (oldSetting, newSetting) {
  ...
  function toHexString(val) {
    if (isArray(val)) {
      var converted = [];
      each(val, function(v, value) {
        converted.push(toHexString(value));
      });
      return converted;
    }
  ...
  function toCallbackOnThisVisualization(val, prop) {
    return val && asGlobalCallback(function () {
      return val.apply(implementationVisualization, arguments);
    }, prop);
  }
  ...
  function toCallbackWithTransformedResult(transformer) {
    return function(callback, property) {
      return asGlobalCallback(function() {
        var result = callback && callback.apply(implementationVisualization, arguments);
        return transformer.call(implementationVisualization, result, property);
      }, property);
    };
  }
  ...
  function expandAll() { return element.expandAll(); }
  function expand(group) { return element.setOpen(group, true); }
  function collapseAll() { return element.collapseAll(); }
  function collapse(group) { return element.setOpen(group, false); }
  function updateColors() { return element.updateColorModel(); }
  function getRegion(group) { return element.getRegion(group); }
