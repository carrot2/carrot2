    ...
    extends AbstractTestCase
{
        ...
        throws MalformedURLException, IOException, SAXException
    {
        ...
        WebResponse response = wc.getResponse(
            super.getControllerURL().toExternalForm() + "/index.jsp?query=data%20mining&processingChain=carrot2.procss.lsi-alltheweb-en&resultsRequested=100");
        ...
        throws MalformedURLException, IOException, SAXException
    {
        ...
        WebResponse response = wc.getResponse(
        super.getControllerURL().toExternalForm() + "/newsearch.do?query=data+mining&processingChain=carrot2.process.html-display.scoresort.x-carrot-clustering-groups&resultsRequested=100");
        assertTrue( "Two frames in the response", response.getFrameNames().length == 2);
        response = wc.getFrameContents("output");
        assertEquals("Response code 200", 200, response.getResponseCode());
            ...
            for (int i=0;i<links.length;i++)
            {
                if (links[i].getURLString().indexOf("query=") != -1)
                {
                    ...
                    if (response.getText().indexOf("*C2TESTSERRORPAGE*") >= 0)
                    {
                        ...
                        errors.append("Component error page returned for URL: " + decodedURL + "\n");
                    }
                    if (response.getText().indexOf("*PAGE_RENDERED_CORRECTLY*") < 0)
                    {
                        // this is not the right input, mate...
                        log.error("Suspicious output (not ending with '*PAGE_RENDERED_CORRECTLY*') for URL: " + decodedURL + "\n");
                        errors.append("Suspicious output (not ending with '*PAGE_RENDERED_CORRECTLY*') for URL: " + decodedURL + "\n");
                    }
                }
            }
            if (errors.length() > 0)
