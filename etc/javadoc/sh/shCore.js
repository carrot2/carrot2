...
var sh = {
	...
	all : function(params)
	{
		...
		sh.utils.addEvent(
			window,
			'load',
			function() { sh.highlight(params); sh.fixContentSpans();}
		);
	}
}; // end of sh
...
sh.Highlighter.prototype = {
	...
	createDisplayLines : function(code)
	{
		var lines = code.split(/\n/g),
			...
			useLineNumbers = this.getParam('gutter')
			;
		...
		for (var i = 0; i < lines.length; i++)
		{
			...
			if (useLineNumbers) {
				var lineNumber = sh.utils.padNumber(firstLine + i, padLength);
				lineNumberCode = '<code class="number">' + lineNumber + '.</code>';
			}
