...
/**
 * A base ANT task for adding, replacing or listing licensing info
 * in class headers or footers. 
 */
public abstract class AbstractLicenseTask extends MatchingTask {

    /**
     * A list of <code>FileSet</code> indicating
     * files with <code>*.java</code> suffix.
     */
    ...
    /**
     * If set to <code>true</code>, the task will accept files not
     * ending with <code>.java</code>.
     */
        ...
        throws BuildException
    {
        ...
        if (getEncoding() == null) {
            throw new BuildException("Source file encoding is required (an explicit attribute)");
        } else {
            // Check if this encoding exists.
            try {
                // We log the message and at the same time check if the encoding
                // is all right.
                super.log("Source files encoding is: "
                        + new String(getEncoding().getBytes(getEncoding()), getEncoding()), Project.MSG_VERBOSE);
        ...
        for (Iterator i = filesets.iterator(); i.hasNext();) {
            ...
            for (int fIndex = 0; fIndex < srcFiles.length; fIndex++) {
                ...
                if (false == forceAllFiles && false == file.getName().endsWith(".java")) {
                    throw new BuildException("File does not end with '.java'. Use 'force' attribute to override.");
                }
    ...
    /**
     * Set to <code>true</code> to make the task accept files not ending with
