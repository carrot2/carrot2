...
/**
 * This class implements a generic algorithm for creating a SuffixTree.
 * The algorithm has been derived from the C-implementation by Mark Nelson: <PRE>
 *   <B>Fast String Searching With Suffix Trees
 *   <I>Mark Nelson</i></B>
 *   Dr Dobb's Journal, August 1996
 *   http://softlab.od.ua/algo/data/suftrees/suffixt.htm (2000)</PRE>
 *
 * The algorithm itself was proposed by Esko Ukkonnen.
 *
 * This class can build suffix trees from any objects which implement
 * <code>SuffixableElement</code>
 * interface and object's elements must implement <code>java.lang.Comparable</code>
 * interface. <code>hashCode()</code> and <code>equals()</code>
 * functions <B>may</B> be applied to container's elements.
 *
 * It is obvious that the performance of this class will <B>highly</B> depend on
 * the type of collection passed. In general, collection objects will be accessed randomly, so
 * a linked list is perhaps not the best choice. An <code>ArrayList</code> object would be a
 * better solution (or
 * a custom wrapper for an array if needed). Also, the time needed for casts and RTTI may be
 * significant - if case a faster implementation of this class is needed, it can always be subclassed
 * to a specific type/ JNI implementation.
 *
 * @author Dawid Weiss
 */
public class SuffixTree
{
    ...
    /**
     *  Nodes factory. Required if this class is subclassed and new type of Node is
     *  required.
     */
    ...
    public synchronized Node add(SuffixableElement element)
    {
        if (this.currentSuffixableElement != null)
            throw new RuntimeException("This class doesn't implement generalized SuffixTrees.");
    ...
    /**
     *  The heart of Ukkonnen's algorithm. Inserts a single prefix to a SuffixTree
     *  while retaining its structure.
     *
     *  The source code to this function is commented with what Mark Nelson had in his excellent
     *  paper published in Dr Dobb's journal.
     *
     *  @param active Active point in the suffix tree
     *  @param lastElementIndex The index of currently inserted element from getCurrentElement().
     *  @return current endpoint (becomes active point if more elements are to be inserted).
     */
    protected Suffix insertPrefix(Suffix active, int lastElementIndex)
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                // edge and create a new node IF the implicit node doesn't match inserted element.
                edge = active.originNode.findEdgeMatchingFirstElement(active.firstElementIndex);

                int span = active.lastElementIndex - active.firstElementIndex;

                if (edge.getEndNode().getSuffixableElement().get(edge.firstElementIndex + span
                        + 1).equals(getCurrentElement().get(lastElementIndex)))
                {
                    break;
                }
            ...
            //
            parent_node.createChildNode(lastElementIndex,
     ...
     * structure on which this object operates - any changes will reflect in the state
     * of the SuffixTree.
     */
