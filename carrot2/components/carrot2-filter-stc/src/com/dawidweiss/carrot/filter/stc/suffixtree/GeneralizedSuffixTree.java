...
/**
 * This class implements a Generalized Suffix Tree data structure. GST is a Suffix tree capable
 * of storing suffixes for more than one SuffixableElement.
 *
 * A specialized Node type is used in this subclass called simply GSTNode.
 *
 * @author Dawid Weiss
 */
    ...
    extends SuffixTree
{
    ...
    public Node add(SuffixableElement element)
    {
        ...
        else
        {
            ...
            // find out at which node the longest matching prefix of current SuffixableElement
            ...
            for (startIndex = 0; startIndex < getCurrentElement().size(); )
            {
                ...
                follow = lastPrefixNode.findEdgeMatchingEntirely(getCurrentElement(), startIndex);

                if (follow == null)
                {
                    endIndex = startIndex - 1;

                    // check if it maybe was an implicit node somewhere along the edge
                    ...
                    for (int i = follow.getStartIndex(); i <= follow.getEndIndex(); i++)
                    {
                                ...
                                this.getCurrentElement().get(endIndex + 1)) == false)
                        {
                            ...
                            // first non-matching character (implicit node on compressed path).
                            break;
                        }
            ...
            if (startIndex == getCurrentElement().size())
            {
                ...
                for (int i = 0; i < getCurrentElement().size(); i++)
                {
                    ...
                    ((GSTNode) activePoint.originNode).addIndexedElement(getCurrentElementNumber());
                }
