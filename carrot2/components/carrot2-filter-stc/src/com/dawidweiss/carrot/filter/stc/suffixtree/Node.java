...
public class Node
{
    ...
    /**
     *  Edges leaving this node. Please note that this way of
     *  storing edges may reduce performance of the algorithm.
     *  This is a generic implementation - if faster storage is needed,
     *  subclass this class.
     */
    ...
    /**
     *  This field holds the edge from parent node to this node.
     *  This is to ease later management of this suffix tree.
     */
    ...
    /**
     * Finds an edge starting in this node and labelled with
     * the particular element from current container's SuffixableElement.
     * Null is returned if no edge has been found.
     */
    ...
    /**
     * Returns an iterator object of edges leaving this Node. Please notice
     * that using remove() method of this iterator may not throw an exception
     * and despite ruin suffix tree structure.
     */
     ...
     * if the Edge holds three elements, the three of them are matched against elements
     * of SuffixableElement at index, index+1 and index+3.
     *
    ...
    public Edge findEdgeMatchingEntirely(SuffixableElement t, int startIndex)
    {
        ...
        if (matchingEdge != null)
        {
            ...
            // calculate the number of elements to compare. if there are less elements in t
            // than in the edge, it may be possible that suffixableElement ends somewhere
            // along the compressed path (is a prefix of the suffix represented by this path).
            ...
            if (matchingEdge.getEndIndex() > matchingEdge.getStartIndex() + maxLength - 1)
            {
                ...
                // we are along compressed path. Return null because we need entirely matching
                // edge.
                return null;
            }
        }
    ...
    /**
     * Creates a child note and returns an edge to it. You need to provide
     * the element on which the edge is created.
     */
    ...
    /**
     * Returns the index of the start of this node's suffix (substring actually).
     * To obtain a full path from root to this node, traverse SuffixableElement like
     * below:
     *
     * <CODE>
    ...
    /**
     * Returns the index of the last element of this node's suffix.
     * To obtain a full path from root to this node, traverse SuffixableElement like
     * below:
     *
     * <CODE>
        ...
        extends AbstractList
    {
        ...
        public Phrase()
        {
            ...
            if (Node.this.getSuffixableElement().get(end) 
                    == SuffixableElement.END_OF_SUFFIX)
            {
                // skip EOS marker
                end--;
            }
    ...
    /**
     * Creates a Collection object which encapsulates the phrase ending at this node.
     */
    ...
    {
        edges.put(edge.getEndNode().getSuffixableElement().get(edge.firstElementIndex), edge);
    }
    ...
    {
        edges.remove(edge.getEndNode().getSuffixableElement().get(edge.firstElementIndex));
    }
    ...
    public String bpath()
    {
        if (this.suffixNode == null) return this.toString() + " [end]";
        else return this.toString() + "-->" + this.suffixNode.bpath();
    }
    ...
    /** Appends contents of this Node and subnodes to a stringbuffer (with certain indent) */
