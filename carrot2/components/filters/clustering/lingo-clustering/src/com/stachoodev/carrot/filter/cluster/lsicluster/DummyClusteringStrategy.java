 ...
 *
 */
public class DummyClusteringStrategy
    implements ClusteringStrategy
{
    ...
    public ClusteringResults cluster(AbstractClusteringContext clusteringContext)
    {
        ...
        {
            if (
                ((features[i].getLength() > 1) && (features[i].getTf() > 1))
        ...
        Matrix tdMatrix = new Matrix(tdMatrixBuildingStrategy.buildTdMatrix(clusteringContext));
...
//		MatrixUtils.normalizeColumnLengths(b);
        if (firstPhrase < features.length) // we need some phrases
        {
            ...
            Matrix phraseMatrix = new Matrix(termCount, features.length - firstPhrase);
            ...
            for (int p = firstPhrase; p < features.length; p++)
            {
                ...
                for (int f = 0; f < featureIndices.length; f++)
                {
                    ...
                    {
                        phraseMatrix.set(
                            featureIndices[f], p - firstPhrase,
                            phraseMatrix.get(featureIndices[f], p - firstPhrase) + 1
                        );
                    }
                }
            }
...
//						!contains(features[phraseIndex[c]].getPhraseFeatureIndices(), i))
        ...
        return new ClusteringResults(clusters);
    }
    ...
    private Matrix getReducedTdMatrix(Matrix tdMatrix, int k, int q)
    {
        ...
        Matrix Rs = new Matrix(tdMatrix.getRowDimension(), tdMatrix.getColumnDimension());
        ...
        for (int i = 0; i < k; i++)
        {
            ...
            b = Rs.times(Rs.transpose()).eig().getV().transpose().getMatrix(
                    0, Rs.getRowDimension() - 1, 0, 0
                );
    ...
    private int calculateClusterCount(Matrix tdMatrix)
    {
        ...
        // Eigenvalues
        double [] lambda = tdMatrix.transpose().times(tdMatrix).times(1.0 / (double) m).eig()
                                   .getRealEigenvalues();
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            double numerator = sum / (double) (n - i);
            ...
            // Denominator
            double denominator = Math.pow(product, 1.0 / (double) (n - i));

            // MDL
            double currentMdl = (m * (n - i) * (Math.log(numerator / denominator)))
                + ((i * ((2 * n) - i + 1) * Math.log(m)) / 2);

            System.out.println(i + " " + currentMdl + " " + denominator + " " + numerator);
