    ...
    implements FeatureExtractionStrategy
{
    ...
    public Feature [] extractFeatures(AbstractClusteringContext clusteringContext)
    {
        ...
        snippetsIntWrapper = new DefaultSnippetsIntWrapper(preprocessedSnippets, stopWords);
        snippetsIntWrapperReversed = new DefaultSnippetsIntWrapper(preprocessedSnippets, stopWords);
        ...
        for (int i = 0; i < phraseTerms.length; i++)
        {
            ...
            StringBuffer unstemmed = new StringBuffer(
                    StringUtils.capitalize(singleTerms[phraseFeatureIndices[0]].getText())
                );
        ...
        for (int i = 0; i < (intData.length - 1); i++) // intData ends with -1
        {
            if (intData[i] < termCount) // skip delimiter codes (0x7fffffff)
            {
                if (singleTerms[intData[i]] == null)
                {
                    ...
                    {
                        singleTerms[intData[i]].setText(
                            StringUtils.capitalize((String) textStemmed)
                        );
                    }

                    // Mark stopwords
                    if (stopWords.contains(singleTerms[intData[i]].getText().toLowerCase()))
                    {
                        singleTerms[intData[i]].setStopWord(true);
                        singleTerms[intData[i]].setText(
                            singleTerms[intData[i]].getText().toLowerCase()
                        );
                    }
