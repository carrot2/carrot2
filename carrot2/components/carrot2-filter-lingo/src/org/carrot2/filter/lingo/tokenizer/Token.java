...
public class Token {
     ...
     * is determined by JavaCCParser, and a table of these numbers is stored
     ...
     * token manager has not read tokens beyond this one, this field is set to
     * null.  This is true only if this token is also a regular token.
     ...
     * (without an intervening regular token).  If there is no such token,
     * this field is null.
     */
     ...
     * add the cases to the switch for all those special cases. For example,
     * if you have a subclass of Token called IDToken that you want to create
     * if ofKind is ID, simlpy add something like : case MyParserConstants.ID
