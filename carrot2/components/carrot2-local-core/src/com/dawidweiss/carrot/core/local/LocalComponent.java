...
/**
 * Local component interface identifies objects that can be assembled in
 * "local" query processing chains. These local processing chains are designed
 * to allow components to directly communicate with each other via method
 * calls, avoiding the overhead of network data streaming and serialization/
 * deserialization cost present in the general Carrot2 framework design.
 * 
 * <p>
 * The {@link LocalComponent} interface is the superclass for all three types
 * of local components:
 * 
 * <ul>
 * <li>
 * {@link LocalInputComponent} -- components accepting user queries and
 * producing initial data.
 * </li>
 * <li>
 * {@link LocalFilterComponent} -- components somehow altering or enriching the
 * data.
 * </li>
 * <li>
 * {@link LocalOutputComponent} -- components gathering the result or doing
 * something with the result. For example, a visual component displaying the
 * data can implement this interface.
 * </li>
 * </ul>
 * </p>
 * 
 ...
 * <ul>
 * <li>
 * {@link LocalComponent#startProcessing(RequestContext)}
 * </li>
 * <li>
 * {@link LocalComponent#endProcessing()}
 * </li>
 * <li>
 * {@link LocalComponent#processingErrorOccurred()}
 * </li>
 * <li>
 * {@link LocalComponent#flushResources()}
 * </li>
 * </ul>
 * 
 ...
 * lifecycle should be respected by any containers or  independent
 * applications that instantiate components:
 * 
 * <ul>
 * <li>
 * {@link #init(LocalControllerContext)} method is invoked right after the
 * component has been instantiated. Upon returning from this method, the
 * component should be ready to process queries.
 * </li>
 * <li>
 * The component is ready to process queries. {@link LocalProcess} instances
 * will link the component in a processing chain with other components and
 * invoke the following methods: {@link #startProcessing(RequestContext)},
 * {@link #endProcessing()}, {@link #processingErrorOccurred()} and {@link
 * #flushResources()}. The order of invokation of these methods is explained
 * in the contract of the {@link LocalProcess} interface.
 * </li>
 * </ul>
 * </p>
 * 
 ...
 * capabilities match <tt>B</tt>'s exposed capabilities. A set of capabilities
 * <tt>X</tt> matches another set of capabilities <tt>Y</tt> if for any object
 * <tt>x</tt> in <tt>X</tt> there exist at least one object <tt>y</tt> in
 * <tt>Y</tt> for which <code>x.equals(y)</code> method returns
 * <code>true</code>. Equality is defined as in {@link
 * java.lang.Object#equals(Object)}.
 * </p>
 * 
 * <p>
 * The operation of a component may be customized using <i>properties</i>.
 * Properties passed to a component via {@link #setProperty(String key, String value)}
 * method are persistent, i.e. they will be active if the component is reused. It
 * is therefore reasonable to set these properties in {@link LocalComponentFactory}
 * at the time of component's instantiation. Properties
 * passed to the component in {@link RequestContext} are volatile, i.e.
 * they are valid only for the duration of the currently processed request. 
 * </p>
 *
 * @author Dawid Weiss
 * @version $Revision$
...
public interface LocalComponent {
     ...
     *         properly initialize and should not be used to process queries.
     */
     ...
     * 
     * <p>
     * It is important to <b>invoke
     * <code>startProcessing(RequestContext)</code> method on any successor
     * components that this component may have</b>. This "chained" invocation
     * is necessary to ensure that processing chain's  successive components
     ...
     *
     * @throws ProcessingException Thrown if the component encountered a
     *         problem in processing the query. May also be thrown from a
     *         chained successor component.
     */
    public void startProcessing(RequestContext requestContext)
        throws ProcessingException;

    /**
     * Signals that the chain's predecessor components ended processing and
     * that no more data will be passed to the component. The component should
     * end its own processing, invoke any necessary data-related methods on
     * the successor component and finally invoke the
     * <code>endProcessing()</code> method to finalize processing  in
     * subsequent components.
     *
     * @throws ProcessingException Thrown if the component encountered a
     *         problem in processing the query. May also be thrown from a
     *         chained successor component. It is allowed to throw an
     *         exception and not invoke <code>endProcessing()</code> method on
     ...
     * LocalComponent}'s class documentation for a more detailed description
     * of capabilities.
     *
     * @return A {@link java.util.Set} object with capabilities. The object
     *         returned may be empty, but should never be null.
     ...
     * method remain valid for all subsequent queries (in case the component
     * is reused).
     * 
     * <p>
     * It is reasonable to set the persistent properties of a component at the
     * time of its instantiation (in the {@link LocalComponentFactory}
     * object).
     * </p>
     * 
