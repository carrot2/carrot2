 ...
 * 
 * @author Dawid Weiss
 * TODO: This class should be replaced with Dom4J's XPath support
 */
public class JDOMHelper {

    /**
     * No instantiation needed - all methods are static anyway, but if somebody
     * insists (for the sake of shorter names or performance issues - dynamic
     * calls are faster than static invocation)
     */
    public JDOMHelper() {
    }

    /**
     * Retrieves a JDOM's Element object from a specified 'path'. Starting from
     * root, children must be delimited by '/' characters, for instance:
     * "general/log4j/layout".
     * 
     * The first path's element must be named the same as the root argument
    ...
    public static List getElements(String path, Element current) {
        ...
        if (someChild != null) {
            ...
            if (someChild.getParent() != null
                    && someChild.getParent() instanceof Element)
                return ((Element) someChild.getParent()).getChildren(someChild
                        .getName());
            else {
     ...
     * '/' characters, attributes separated from paths using hash '#'
     * characters. For instance, if passed the root of JDOM's tree:
     * "start/beginwith#attribute" will descend into start element, look for
     * beginwith element and search for its attribute 'attribute'.
     * 
     * @return a string or null if not succeeded and exceptionIfNotPresent not
     *         set.
     * @throws IllegalArgumentException
     *             if exceptionIfNotPresent argument is set to true and the path
     *             could not be found.
     */
    public static String getStringFromJDOM(String path, Element root,
            boolean exceptionIfNotPresent) {
        ...
        if (e == null) {
            if (exceptionIfNotPresent) {
                throw new IllegalArgumentException("Path element: " + element
                        + " not present in: " + root);
            }
            return null;
        }
        ...
        if (attributeSelector != -1) {
            try {
                ...
                throw new IllegalArgumentException(
                        "Empty attribute name is illegal.");
            }
        } else {
            return e.getText();
        }
    ...
    /**
     * Extracts the string of an element, or an attribute from JDOM's XML tree.
     * Same as getStringFromJDOM(String,Element,boolean), but instead of
     * returning null for non-existing paths/ values, it returns the default
     * value.
     * 
    ...
    public static Document buildDocumentFromString(String xml,
            boolean validation) throws JDOMException, IOException {
     ...
     */
    public static Document buildDocumentFromStream(InputStream xmlStream,
            boolean validation) throws JDOMException, IOException {
        SAXBuilder builder = new org.jdom.input.SAXBuilder(validation);
