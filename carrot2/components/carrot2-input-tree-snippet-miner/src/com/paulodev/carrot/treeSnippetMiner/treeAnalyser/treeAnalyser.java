...
public class treeAnalyser
{
    ...
    public Element process()
    {
        ...
        XMLOutputter ou = new XMLOutputter(format);
    ...
    switch (t.type) {
            ...
            case Token.TYPE_ZONE:
                if (t.start.getAttribute(name) != null && t.start.getAttribute(name).getValue().equals("endBefore"))
 //                   t.start.removeAttribute(name);
    ...
    private void tryRemoveElement(Element e) {
        if ((e.getName().equalsIgnoreCase("b") ||
            ...
            ((Element) e.getParent()).getChildren().remove(e);
        }
