...
import org.put.util.exception.WrappedException;
import org.put.util.xml.JDOMHelper;


/**
 * Objects of this class describe how to extract a single snippet from some string using regular
 * expressions.
 *
 * @author Dawid Weiss
 */
public class SnippetDescription
{
    ...
    /**
     * Initializes this object using an XML Element. The DOM structure must contain at least the
     * following elements (this is an example of wrapping Google).
     * <pre>
        ...
        throws WrappedException
    {
        ...
        try
        {
            snippetMatch = new RegularExpression(JDOMHelper.getElement(SNIPPET_MATCH_ALL, snippet));
            titleStart = new RegularExpression(JDOMHelper.getElement(TITLE_START, snippet));
            titleEnd = new RegularExpression(JDOMHelper.getElement(TITLE_END, snippet));
            urlStart = new RegularExpression(JDOMHelper.getElement(URL_START, snippet));
            urlEnd = new RegularExpression(JDOMHelper.getElement(URL_END, snippet));
            summaryStart = new RegularExpression(JDOMHelper.getElement(SUMMARY_START, snippet));
            summaryEnd = new RegularExpression(JDOMHelper.getElement(SUMMARY_END, snippet));
        }
        catch (IllegalArgumentException e)
        {
            throw new WrappedException(
                "Could not create SnippetDescription: some parameters missing.", e
            );
        }
...
/**
 * Regular expression token holder. Holds the regular expression string and an indicator whether
 * the match should be consumed, or included in the result.
 */
class RegularExpression
{
    ...
    public RegularExpression(Element regExpXmlElement)
    {
        ...
        if (regExp == null)
        {
            throw new IllegalArgumentException("XML Element does not contain required attributes.");
        }

        if (regExpXmlElement.getAttribute(XML_ATTR_CONSUME) != null)
        {
            if ("true".equalsIgnoreCase(regExpXmlElement.getAttribute(XML_ATTR_CONSUME).getValue()))
            {
                consumeToken = true;
            }
        }
