...
package org.put.snippetreader.extractors.regexp;
...
public class RegExpSnippetExtractor
{
    ...
    /**
     * Processes an input stream and extracts snippets. <b>Closes the input reader on exit (even if
     * an exception has been thrown)</b>
     *
     * @param inputCharacterStream An input stream to be parsed.
     * @param callback A callback object, which will be notified during the parsing of the input
     *        stream.
     *
     * @throws InterruptedException
     * @throws REException
     */
    public void extractSnippets(Reader inputCharacterStream, SnippetExtractorCallback callback)
        throws REException, InterruptedException
    {
        ...
        try
        {
            ...
            RE snippetRazor = new RE(
                    snippetDescription.getSnippetMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            RE titleStartMarker = new RE(
                    snippetDescription.getTitleStartMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            RE titleEndMarker = new RE(
                    snippetDescription.getTitleEndMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            RE URLStartMarker = new RE(
                    snippetDescription.getURLStartMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            RE URLEndMarker = new RE(
                    snippetDescription.getURLEndMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            RE summaryStartMarker = new RE(
                    snippetDescription.getSummaryStartMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            RE summaryEndMarker = new RE(
                    snippetDescription.getSummaryEndMatch().getRegExp(), RE.REG_DOT_NEWLINE
                );
            ...
            while (matches.hasMoreMatches())
            {
                ...
                title = extractSubExpression(
                        fullSnippet, titleStartMarker, titleEndMarker,
                        snippetDescription.getTitleStartMatch().isConsumeToken(),
                        snippetDescription.getTitleEndMatch().isConsumeToken()
                    );

                callback.entireSnippetRegionMatch(
                    fullSnippet, matchingElement.getStartIndex(), matchingElement.getEndIndex()
                );
                ...
                url = extractSubExpression(
                        fullSnippet, URLStartMarker, URLEndMarker,
                ...
                summary = extractSubExpression(
                        fullSnippet, summaryStartMarker, summaryEndMarker,
                        snippetDescription.getSummaryStartMatch().isConsumeToken(),
     ...
     *
     * @param expression A full string from which a subexpression is to be extracted.
     ...
     * @param consumeStart Consume the regular expression at the start of subexpression?
     * @param consumeEnd Consume the regular expression at the end of subexpression?
     *
     * @return
     */
    protected String extractSubExpression(
        String expression, RE startMarker, RE endMarker, boolean consumeStart, boolean consumeEnd
    )
    {
        REMatch start;
        REMatch end;
        ...
        subExprStart = (consumeStart ? start.getEndIndex()
                                     : start.getEndIndex());
        subExprEnd = (consumeEnd ? end.getStartIndex()
                                 : end.getEndIndex());
