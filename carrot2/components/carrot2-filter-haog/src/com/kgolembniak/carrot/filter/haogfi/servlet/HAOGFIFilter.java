...
package com.kgolembniak.carrot.filter.haogfi.servlet;
...
import com.kgolembniak.carrot.filter.haogfi.local.HAOGFILocalFilterComponent;
 ...
 * Groups on Apriori clustering algorithm.<br/>
 ...
 */
public class HAOGFIFilter extends FilterRequestProcessor {
	
	private final Logger logger = Logger.getLogger(this.getClass());
    /**
     * Name of the local process used for executing remote queries.
     */
    private final static String LOCAL_PROCESS_ID = "haog-fi";
			...
			Map paramsBeforeData) throws Exception
	{
        ...
        // Run local process.
        logger.debug("Running local HAOG-FI clustering process.");
    ...
    /**
     * Set up Local controller configured to cluster with FI. 
     */
    private final static LocalController setupController() {
        ...
        final LocalComponentFactory haogFilter = 
        	new LocalComponentFactoryBase() {
            public LocalComponent getInstance() {
                return new HAOGFILocalFilterComponent();
            }
        };
        controller.addLocalComponentFactory("filter.haog-fi", haogFilter);
        ...
        try {
            final LocalProcessBase hagostcProcess = new LocalProcessBase(
                ...
                "local-haog-fi-process", "Local HAOG-FI process " +
