...
SearchTabView.prototype.applyTabInsert = function(tab) {
  ...
  var bodyEl = tds[tds.length - 2].cloneNode(false);
  var linkEl = tds[tds.length - 1].cloneNode(false);

  var tabEl = document.getElementById(tab.id).cloneNode(true);
  bodyEl.appendChild(tabEl);
  rowElement.insertBefore(linkEl, tds[tds.length - 2]);
  rowElement.insertBefore(bodyEl, tds[tds.length - 2]);
