 ...
 * </p>
 * <P class="classinfo">
 * A slight modification of java.util.Vector class that gives some more freedom in
 * menaging of vector contents. First change is that there CAN be empty gaps "inside" a
 * vector (at some index). Second change is that array of objects is now in public scope.
 * Third there are some minor methods for sorting (using java.util.Array) and binary
 * search over sorted vector.
 * </p>
 * <P class="classinfo">
 ...
 * NON-COMMERCIAL purposes and without fee is hereby granted provided that this copyright
 * notice appears in all copies. Please refer to the file "copyright.html" for further
 * important copyright and licensing information.
 * </p>
 * <P class="disclaimer">
 * SUN MAKES NO REPRESENTATIONS OR WARRANTIES ABOUT THE SUITABILITY OF THE SOFTWARE,
 * EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTIES OF
 * MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT. SUN SHALL NOT
 ...
 *          1.0, 18 Apr 1999, Rewritten documentation.<BR>
...
final class FastVector extends AbstractList implements Cloneable, List
{
    ...
    /**
     * The index of the last element. This field is actually vector's length. For example
     * when you have a vector like this ["a","b","c"] then the elementCount would be 3 and
     * lastIndex would be 2 (because indexes are zero-based). But if you erased two first
     * elements, leaving [ null, null, "c" ] then elementCount=1 (because there is only
     * one element in the vector!) however lastIndex would still remain 2. After you erase
     * "c" lastIndex would be set to -1 (so it's enough to add 1 to lastIndex to determine
     * vector's length.
     */
    ...
    /**
     * Returns the internal array of objects which this vector operates on. Any changes to
     * that array immediately affect this vector.
     */
    ...
    /**
     * Returns the index of the last not-null element in the vector, starting from
     * position <code>position</code> downwards.
     * 
    ...
    /**
     * Sets the size of the vector. If the size shrinks, the extra elements (at the end of
     * the vector) are lost; if the size increases, the new elements are set to null.
     * 
     ...
     * vector's capacity or last index! The size of a vector is not enough to enumerate
     * vector contents using for loop! Please look at this simple example: <blockquote>
     * 
     * <pre>
     ...
     * </p>
     * <p>
     * Now it'll print out: "null,b". As you can see it makes little sense because we have
     * element "c" that is not printed. The correct loop should look like this:
     * <blockquote>
     * 
     * <pre>
    ...
    /**
     * Returns last element's index in the vector. Note that this is not the same as the
     * vector's size or capacity.
     * 
     * @see #size
     */
    ...
    /**
     * Returns true if the specified object is a value of the collection. This method is
     * here because of compatibility reasons (it existed in original Vector class). In
     * FastVector you can use binary search (after sorting) and it's a lot faster then
     * scanning linearily the vector's contents (what this method does).
     * 
    ...
    /**
     * Searches for the specified object, starting at the specified position and returns
     * an index to it.
     * 
     * @param elem the desired element
    ...
    public Object elementAt(int index)
    {
        ...
        /*
         * Since try/catch is free, except when the exception is thrown, put in this extra
         * try/catch to catch negative indexes and display a more informative error
         * message. This might not be appropriate, especially if we have a decent
         * debugging environment - JP.
         */
        try
        {
    ...
    /**
     * Sets the element at the specified index to be the specified object. The previous
     * element at that position is discarded.
     * 
    ...
    /**
     * Deletes the element at the specified index. Elements with an index greater than the
     * current index are <B>NOT</B>moved down! It's a difference when compared to
    ...
    /**
     * Inserts the specified object as an element at the specified index. Elements with an
     * index greater or equal to the current index are shifted up.
     * 
    ...
    public void insertElementAt(Object obj, int index)
    {
        ...
        System.arraycopy(elementData, index, elementData, index + 1, (lastIndex + 1)
            - index);
    ...
    /**
     * Adds the specified object as the last element of the vector. It doesn't look up
     * empty "slots" or anything like that - the element is added after the element
     * pointed by lastIndex.
     * 
    ...
    /**
     * Removes the element from the vector. If the object occurs more than once, only the
     * first is removed. If the object is not an element, returns false.
     * 
    ...
    /**
     * Removes all null objects (empty slots) from inside a vector. This method changes
     * relative indexes and shifts objects up if it's necessary. Requires one pass through
    ...
    public String toString()
    {
        ...
        {
            String s = (elementData[i] == null) ? "<null>" : elementData[i].toString();
    ...
    /**
     * Sorting a vector in ascending order. This method will raise an exception if objects
     * are not comparable. All objects MUST implement Comparable interface. Sorting order
     * is ascending only in terms of values returned from
     * <code>Comparable.compareTo</code> method. By smart implementation of this method
     * you can sort objects in any order you like.
     * 
     * @see Comparable
     * @see #bsearch
     */
    ...
    /**
     * Performs a binary search over previously sorted data. Note that if the vector is
     * not sorted the results are unpredictible. Objects MUST implement Comparable
     * interface.
     * 
     * @param key an object that is to be found in the vector.
     * @return Index of key's occurence in vector or -1 if vector doesn't contain such
     *         key. Note that if there are many object matching key then the index points
     *         to one of these objects, not necessarily the first occurence.
     * @see Comparable
     * @see #sort
     * @see #removeGaps
     * @see #contains
