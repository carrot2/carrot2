...
/**
 * <p>
 * This class is an implementation of {@link java.util.BitSet} enhanced with direct access
 * to some internal data structures and a few extra operations.
 * <p>
 * I had to copy-paste the code from JDK 1.3's <code>java.util.BitSet</code> because I
 * needed access to private fields of it. I don't know if it violates any of SUN's
 * licenses - if so, the class will have to be rewritten in the future. Below is the
 * original Sun's javadoc for BitSet, please note that ALL references and functions
 * arguments have been replaced with {@link ExtendedBitSet} instead of {@link BitSet}
 * (regardless of what the documentation in this file might say). This class implements a
 * vector of bits that grows as needed. Each component of the bit set has a
 * <code>boolean</code> value. The bits of a <code>BitSet</code> are indexed by
 * nonnegative integers. Individual indexed bits can be examined, set, or cleared. One
 * <code>BitSet</code> may be used to modify the contents of another <code>BitSet</code>
 ...
 * <p>
 * Every bit set has a current size, which is the number of bits of space currently in use
 * by the bit set. Note that the size is related to the implementation of a bit set, so it
...
public final class ExtendedBitSet implements Cloneable, java.io.Serializable
{
    ...
    private final static int [] acceleratedSum;
    ...
    /**
     * Returns the number of set bits in a bitset which would be created after XOR
     * operation of argument and this ExtendedBitSet.
     * 
     * @return number of set bits in (this ^ set).
     */
    public int numberOfSetBitsAfterXor(ExtendedBitSet set)
    {
        ...
        unitsInCommon = (unitsInUse > set.unitsInUse ? set.unitsInUse : unitsInUse);
    ...
    /**
     * Returns the number of set bits in a bitset which would be created after AND
     * operation of this ExtendedBitSet and the argument passed.
     * 
     * @return number of set bits in (this & set).
     */
    ...
    /**
     * Bitset iterator class. The current implementation is very naive - iteration over
     * all bits of this bitset. Some optimizations would be useful. This iterator is not
     * fail-safe!.
     */
    ...
    /**
     * Returns the Iterator over indices of set bits in this object. The iterator is not
     * fail-safe.
     */
    ...
    /*
     * BitSets are packed into arrays of "units." Currently a unit is a long, which
     * consists of 64 bits, requiring 6 address bits. The choice of unit is determined
     * purely by performance concerns.
     */
    ...
    /**
     * The bits in this BitSet. The ith bit is stored in bits[i/64] at bit position i % 64
     * (where bit position 0 refers to the least significant bit and 63 refers to the most
     * significant bit). INVARIANT: The words in bits[] above unitInUse-1 are zero.
     * 
     * @serial
     */
    ...
    /**
     * The number of units in the logical size of this BitSet. INVARIANT: unitsInUse is
     * nonnegative. INVARIANT: bits[unitsInUse-1] is nonzero unless unitsInUse is zero.
     */
     ...
     * WARNING:This function assumes that the number of units actually in use is less than
     * or equal to the current value of unitsInUse!
     */
    private void recalculateUnitsInUse()
    {
        ...
        for (i = unitsInUse - 1; i >= 0; i--)
            if (bits[i] != 0) break; // this unit is in use!
     ...
     * with indices in the range <code>0</code> through <code>nbits-1</code>. All
     * bits are initially <code>false</code>.
     * 
     * @param nbits the initial size of the bit set.
     * @exception NegativeArraySizeException if the specified initial size is negative.
     */
    public ExtendedBitSet(int nbits)
    {

        /* nbits can't be negative; size 0 is OK */
        if (nbits < 0) throw new NegativeArraySizeException(Integer.toString(nbits));
    ...
    /**
     * Returns the "logical size" of this <code>BitSet</code>: the index of the highest
     * set bit in the <code>BitSet</code> plus one. Returns zero if the
    ...
    {
        if (bitIndex < 0) throw new IndexOutOfBoundsException(Integer.toString(bitIndex));
    ...
    /**
     * Clears all of the bits in this <code>BitSet</code> whose corresponding bit is set
     * in the specified <code>BitSet</code>.
     * 
     * @param set the <code>BitSet</code> with which to mask this <code>BitSet</code>.
     * @since 1.2
     */
     ...
     * <code>true</code> if the bit with the index <code>bitIndex</code> is currently
     * set in this <code>BitSet</code>; otherwise, the result is <code>false</code>.
     * 
     ...
     * This bit set is modified so that each bit in it has the value <code>true</code>
     * if and only if it both initially had the value <code>true</code> and the
     * corresponding bit in the bit set argument also had the value <code>true</code>.
     * 
     * @param set a bit set.
     */
    ...
    /**
     * Performs a logical <b>OR </b> of this bit set with the bit set argument. This bit
     * set is modified so that a bit in it has the value <code>true</code> if and only
     * if it either already had the value <code>true</code> or the corresponding bit in
     * the bit set argument has the value <code>true</code>.
     * 
     * @param set a bit set.
     */
    ...
    /**
     * Returns a hash code value for this bit set. The has code depends only on which bits
     * have been set within this <code>BitSet</code>. The algorithm used to compute it
     ...
     * </pre>
     * 
     * is true. Then the following definition of the <code>hashCode</code> method would
    ...
    /**
     * Returns the number of bits of space actually in use by this <code>BitSet</code>
     ...
     * if and only if the argument is not <code>null</code> and is a <code>Bitset</code>
     * object that has exactly the same set of bits set to <code>true</code> as this bit
    ...
    {
        if (!(obj instanceof ExtendedBitSet)) return false;
     ...
     * equal to it. The clone of the bit set is another bit set that has exactly the same
     * bits set to <code>true</code> as this bit set and the same current size.
     * <p>
     ...
     * <code>BitSet</code> contains a bit in the set state, the decimal representation
     * of that index is included in the result. Such indeces aer listed in order from
     * lowest to highest, separated by ",$nbsp;" (a comma and a space) and surrounded by
