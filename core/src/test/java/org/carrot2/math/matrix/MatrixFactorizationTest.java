...
public class MatrixFactorizationTest extends TestBase {
  ...
  @Test
  public void testSVD() {
    PartialSingularValueDecompositionFactory factory =
        new PartialSingularValueDecompositionFactory();
    factory.setK(2);

    PartialSingularValueDecomposition factorization =
        (PartialSingularValueDecomposition) factory.factorize(A);

    DoubleMatrix2D expectedU =
        new DenseDoubleMatrix2D(
            new double[][] {
              {0, 0.752646},
              {-0.651927, 0},
    ...
    double[] expectedS = new double[] {1.6450472, 1.5621864};
  ...
  public void testUnorderedNMFED() {
    ...
    final NonnegativeMatrixFactorizationEDFactory factory =
  ...
  public void testNMFKL() {
    ...
    checkIterative(expectedU, expectedV, new NonnegativeMatrixFactorizationKLFactory());
  }
  ...
  public void testLNMF() {
    ...
    checkIterative(expectedU, expectedV, new LocalNonnegativeMatrixFactorizationFactory());
  }

  private MatrixFactorization checkIterative(
      DoubleMatrix2D expectedU,
      DoubleMatrix2D expectedV,
      IterativeMatrixFactorizationFactory factory) {
    factory.setK(K);
  ...
  private void check(
      DoubleMatrix2D expectedU, DoubleMatrix2D expectedV, MatrixFactorization factorization) {
  ...
  static class ConstantSeedingStrategyFactory implements SeedingStrategyFactory {
    ...
    private final double[][] constantU =
        new double[][] {
          {0.84319, 0.6119},
          {0.8062, 0.10298},
          {0.85779, 0.15832},
    ...
    private final double[][] constantV =
        new double[][] {
          {0.26868, 0.087422},
          {0.78425, 0.93323},
          {0.38787, 0.25938},
