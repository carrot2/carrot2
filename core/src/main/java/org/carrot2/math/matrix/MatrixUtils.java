...
public class MatrixUtils {
  /**
   * Normalizes column vectors of matrix <code>A</code> so that their L2 norm (Euclidean distance)
   * is equal to 1.0.
   *
   * @param A matrix to normalize
   * @param work a temporary array of <code>A.columns()</code> doubles that will be overwritten with
   *     column's original L2 norms. Supply a non-null pointer to avoid continuous
   *     allocation/freeing of memory when doing calculations in a loop. If this parameter is <code>
   *     null</code>, a new array will be allocated every time this method is called.
  ...
  /**
   * Normalizes column vectors of a sparse matrix <code>A</code> so that their L2 norm (Euclidean
  ...
  /**
   * Normalizes column vectors of matrix <code>A</code> so that their L1 norm is equal to 1.0.
   *
   * @param A matrix to normalize
   * @param work a temporary array of <code>A.columns()</code> doubles that will be overwritten with
   *     column's original L1 norms. Supply a non-null pointer to avoid continuous
   *     allocation/freeing of memory when doing calculations in a loop. If this parameter is <code>
   *     null</code>, a new array will be allocated every time this method is called.
  ...
  /**
   * Computes the orthogonality of matrix A. The orthogonality is computed as a sum of k*(k-1)/2
   * inner products of A's column vectors, k being the number of columns of A, and then normalized
   * to the 0.0 - 1.0 range.
   *
   * @param A matrix to compute orthogonality for, must be column length-normalized
   * @return orthogonality of matrix A. 0.0 denotes a perfect orthogonality between every pair of
   *     A's column. 1.0 indicates that all columns of A are parallel.
   */
  ...
  /**
   * Computers sparseness of matrix <code>A</code> as a fraction of non-zero elements to the total
   * number of elements.
   *
   * @return sparseness of <code>A</code>, which is a value between 0.0 (all elements are zero) and
   *     1.0 (all elements are non-zero)
   */
  ...
  /**
   * Finds the first minimum element in each column of matrix A. When calculating minimum values for
   * each column this version should perform better than scanning each column separately.
   *
   * @param indices an array of <code>A.columns()</code> integers in which indices of the first
   *     minimum element will be stored. If this parameter is <code>null</code> a new array will be
   *     allocated.
   * @param minValues an array of <code>A.columns()</code> doubles in which values of each column's
   *     minimum elements will be stored. If this parameter is <code>null</code> a new array will be
   *     allocated.
  ...
  /**
   * Finds the first maximum element in each column of matrix A. When calculating maximum values for
   * each column this version should perform better than scanning each column separately.
   *
   * @param indices an array of <code>A.columns()</code> integers in which indices of the first
   *     maximum element will be stored. If this parameter is <code>null</code> a new array will be
   *     allocated.
   * @param maxValues an array of <code>A.columns()</code> doubles in which values of each column's
   *     maximum elements will be stored. If this parameter is <code>null</code> a new array will be
   *     allocated.
  ...
  public static int[] maxInColumns(
    ...
    return inColumns(A, indices, maxValues, DoubleComparators.NATURAL_ORDER, transform);
  }
   ...
   * @return index of the first maximum element or -1 if the input matrix is <code>null</code> or
   *     has zero size.
   */
   ...
   *
   * @param sums an array to store the results. If the array is <code>null</code> or does not match
   *     the number of rows in matrix <code>A</code>, a new array will be created.
   ...
   *
   * @see <a href="http://en.wikipedia.org/wiki/Matrix_norm#Frobenius_norm">Frobenius norm</a>
   */
  ...
  /**
   * Returns view of the provided matrix with rows permuted according to the order defined by the
   * provided comparator.
   *
