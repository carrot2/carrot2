...
abstract class CompleteLabelFilterBase {
  void filter(
    ...
    final ArrayList<LabelIndexWithCodes> phraseIndexesWithCodes =
        new ArrayList<>(acceptedStems.length + acceptedPhrases.length);
    for (int i = 0; i < acceptedStems.length + acceptedPhrases.length; i++) {
      phraseIndexesWithCodes.add(
          new LabelIndexWithCodes(
              i,
              getLabelWordIndexes(
    ...
    final List<LabelIndexWithCodes> sortedPhrasesWithCodes =
        sortPhraseCodes(phraseIndexesWithCodes);
    ...
    while (i < sortedPhrasesWithCodes.size() - 1) {
      ...
      if (getLabelLength(acceptedStems.length, context.allPhrases.wordIndices, currentLabelIndex)
              == lcpArray[i]
          && isLabelAccepted(
              acceptedStems.length,
              wordsStemIndex,
              currentLabelIndex,
              acceptedStems,
              acceptedPhrases)) {
        ...
        while (j < sortedPhrasesWithCodes.size() - 1 && lcpArray[j] >= lcpArray[i]) {
          ...
          double labelOverride =
              calculateLabelOverride(
                  acceptedStems.length, stemTf, phraseTf, nextLabelIndex, currentLabelIndex);
          if ((isLabelAccepted(
                  acceptedStems.length,
                  wordsStemIndex,
                  nextLabelIndex,
                  acceptedStems,
                  acceptedPhrases)
              && labelOverride >= labelOverrideThreshold)) {
            markLabelAsRemoved(
                acceptedStems.length, currentLabelIndex, acceptedStems, acceptedPhrases);
            break;
          }
  ...
  abstract List<LabelIndexWithCodes> sortPhraseCodes(List<LabelIndexWithCodes> phrasesWithCodes);
  ...
  private static final int[] getLabelWordIndexes(
      int wordCount, int[] mostFrequentWordIndex, int[][] wordIndices, int featureIndex) {
  ...
  private static final boolean isLabelAccepted(
      int wordCount,
      int[] wordStemIndex,
      int featureIndex,
      boolean[] acceptedStems,
  ...
  private static final void markLabelAsRemoved(
      int wordCount, int featureIndex, boolean[] acceptedWords, boolean[] acceptedPhrases) {
    if (featureIndex < wordCount) {
  ...
  private static final double calculateLabelOverride(
      int wordCount,
      int[] wordTf,
      int[] phraseTf,
      int overridingLabelIndex,
      int overridenLabelIndex) {
    final int overridingTf =
        overridingLabelIndex < wordCount
            ? wordTf[overridingLabelIndex]
            : phraseTf[overridingLabelIndex - wordCount];
    final int overridenTf =
        overridenLabelIndex < wordCount
            ? wordTf[overridenLabelIndex]
