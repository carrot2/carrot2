...
/**
 * Sparse array encoding utilities. Sparse means an index and its value are kept in an array as a
 * pair.
 */
public final class SparseArray {
  ...
  public static int[] toSparseEncoding(IntStack documents) {
    if (documents.size() == 0) return EMPTY_INT_ARRAY;
  ...
  public static String sparseToString(int[] intIntArray) {
    ...
    if (intIntArray.length > windowSize) b.append("...");
    return b.toString();
  }
}
