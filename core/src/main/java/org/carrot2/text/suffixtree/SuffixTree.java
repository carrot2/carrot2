 ...
 *
 * @see "E. Ukkonen, On-line construction of suffix trees, Algorithmica, 1995, volume 14, number 3,
 *     pages 249-260."
 */
public final class SuffixTree {
   ...
   *
   * <ul>
   *   <li>at build time, the suffix pointer (state pointer),
   *   <li>after the tree is built, the first edge from a given state (edge pointer).
   * </ul>
   */
  private IntArrayList states = new IntArrayList();

  /**
   * A hash map of transitions (edges) between states in the suffix tree. The map is keyed by a
  ...
  public interface IVisitor {
     ...
     *
     * @return Returning <code>false</code> omits the subtree of <code>state</code>. {@link
     *     #post(int)} is not invoked for this state if skipped.
     */
  ...
  public SuffixTree(
    ...
    for (int i = states.size() - 1; i >= 0; i--) states.set(i, LEAF_STATE);
  ...
  /**
   * Create a transition from state <code>s</code> to state <code>ts</code>, labeled with symbols
   * between <code>k</code> and <code>p</code> (1-based, inclusive).
   */
  ...
  /**
   * Reuse an existing transition slot to store a transition from state <code>s</code> to state
   * <code>ts</code>, labeled with symbols between <code>k</code> and <code>p</code> (1-based,
   * inclusive).
   */
  ...
  /**
   * Adds a transition to state <code>ts</code>, labeled with symbols between <code>k</code> and
   * <code>p</code> (1-based, inclusive), but does not add hash map entry (for internal use).
   */
  ...
  /**
   * Find a transition from state <code>s</code>, labeled with symbol at index <code>k - 1</code> in
   * the input sequence.
   */
  ...
  /**
   * Remove the transition from state <code>s</code>, labeled with symbol at index <code>k - 1
   * </code> and return its slot in the transitions array.
   */
  ...
  /**
   * @return <code>true</code> if this suffix tree has a path from the root state to a leaf state
   *     corresponding to a given sequence of objects. This indicates the input sequence had a
   *     suffix identical to <code>sequence</code>.
   */
  ...
  /**
   * Returns the index of the first edge from a given state or {@link #NO_EDGE} if a given state has
   * no edges. Does not perform any sanity check on the input state.
   */
  ...
  /**
   * Returns the index of the next edge (sibling) or {@link #NO_EDGE} if <code>edge</code> is the
   * last edge in its state.
   */
  ...
  /**
   * Find a transition from state <code>state</code>, labeled with a given symbol. {@link #NO_EDGE}
